...
############################################
# BACKEND SUBNETS – ONE PER AD (FOR INSTANCE POOL)
############################################

# AD-1 backend subnet
resource "oci_core_subnet" "backend_subnet_ad1" {
  compartment_id      = var.compartment_id
  vcn_id              = oci_core_vcn.bharatmart_vcn.id
  cidr_block          = "10.0.2.0/24"
  availability_domain = data.oci_identity_availability_domains.all_ads.availability_domains[0].name

  display_name  = "${var.project_name}-${var.environment}-backend-subnet-ad1"
  dns_label     = "backendad1"
...
# AD-2 backend subnet
resource "oci_core_subnet" "backend_subnet_ad2" {
  compartment_id      = var.compartment_id
  vcn_id              = oci_core_vcn.bharatmart_vcn.id
  cidr_block          = "10.0.3.0/24"
  availability_domain = data.oci_identity_availability_domains.all_ads.availability_domains[1].name

  display_name  = "${var.project_name}-${var.environment}-backend-subnet-ad2"
  dns_label     = "backendad2"
  route_table_id      = oci_core_route_table.private_route_table[0].id
  security_list_ids   = [oci_core_security_list.private_security_list.id]
...
# AD-3 backend subnet
resource "oci_core_subnet" "backend_subnet_ad3" {
  compartment_id      = var.compartment_id
  vcn_id              = oci_core_vcn.bharatmart_vcn.id
  cidr_block          = "10.0.4.0/24"
  availability_domain = data.oci_identity_availability_domains.all_ads.availability_domains[2].name

  display_name  = "${var.project_name}-${var.environment}-backend-subnet-ad3"
  dns_label     = "backendad3"
  route_table_id      = oci_core_route_table.private_route_table[0].id
  security_list_ids   = [oci_core_security_list.private_security_list.id]
...
# Note: Only created when instance pool is NOT enabled
############################################

resource "oci_core_instance" "bharatmart_backend" {
  count               = var.enable_instance_pool ? 0 : var.compute_instance_count
  availability_domain = data.oci_identity_availability_domain.primary.name
  compartment_id      = var.compartment_id
...
# Note: When instance pool is enabled, Load Balancer backends are attached via instance pool configuration.
# See instance-pool-autoscaling.tf for pool configuration.
# You may need to manually attach the instance pool to the Load Balancer backend set via OCI Console or CLI.
