...
locals {
  ...
  # FIX: Updated to include 'metricNameTransform' for the 'bm_' prefix
  backend_cloud_agent_monitoring_config = jsonencode({
    prometheusConfig = {
      ...
      # CRITICAL FIX: Add metricNameTransform to apply the 'bm_' prefix
      metricNameTransform = {
        prefix = "bm_"
      }
    }
  ...
  # FIX: Cloud Agent TRACE configuration for backend (OTLP Ingestion)
  backend_cloud_agent_tracing_config = jsonencode({
    otlp = {
      # This enables the OTLP receiver on the UMA Agent sidecar
      grpc_endpoint = "localhost:4317" # Standard OTLP gRPC port
      http_endpoint = "localhost:4318" # Standard OTLP HTTP port (used by env.tpl)
    }
  })
  backend_cloud_agent_tracing_config_b64 = base64encode(local.backend_cloud_agent_tracing_config)
  ...
  backend_cloud_init = <<-EOF
    ...
    runcmd:
      ...
      - |
        for i in {1..30}; do
          if ! pgrep -x yum > /dev/null && ! pgrep -x dnf > /dev/null;
          then
            break
          fi
      ...
      # FIX: Configure OCI Cloud Agent for OTLP Tracing ingestion
      - |
        mkdir -p /opt/oracle-cloud-agent/plugins/trace
        echo "${local.backend_cloud_agent_tracing_config_b64}" | base64 -d > /opt/oracle-cloud-agent/plugins/trace/config.json

      # FIX: Restart the primary agent (more reliable than unified-monitoring-agent)
  ...
  policies {
    ...
    rules {
      ...
      metric {
        ...
        threshold {
          operator = "GT"
          value    = var.backend_cpu_scale_out_threshold
        }
      }
    }
    ...
    rules {
      ...
      metric {
        ...
        threshold {
          operator = "LT"
          value    = var.backend_cpu_scale_in_threshold
        }
      }
    }
  }
